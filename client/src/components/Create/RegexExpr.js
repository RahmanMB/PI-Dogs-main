...
const regexValidation = (name, value, { height_min, weight_min, changes }, nameDogs) => {
	changes = true;
	switch (name) {
		case "name":
		...
		case "image":
			return urlRegex.test(value) ? "" : `${name} entered is invalid, try another one.`;

		case "life_span":
			return intRangeRegex.test(value)
				? Number(value.split("-").map((el) => el.trim())[0]) < Number(value.split("-").map((el) => el.trim())[1])
		...
		case "height_min":
			return numberRegex.test(value)
				...
				: `${name.split("_").join(" ")} entered is invalid, enter a valid number.`;
		...
		case "height_max":
			return numberRegex.test(value)
				? Number(value) > Number(height_min)
					...
					: `${name.split("_").join(" ")} entered is invalid, enter a valid number greater than ${height_min}.`
				: `${name.split("_").join(" ")} entered is invalid, enter a valid number.`;
		...
		case "weight_max":
			return numberRegex.test(value)
				? Number(value) > Number(weight_min)
					...
					: `${name.split("_").join(" ")} entered is invalid, enter a valid number greater than ${weight_min}.`
				: `${name.split("_").join(" ")} entered is invalid, enter a valid number.`;
